...
# 用“儿童1”至“儿童10”替换原始的儿童姓名
for i, name in enumerate(top_children, start=1):
    child_data = df[df['儿童姓名'] == name].sort_values('测评日期')
    plt.plot(child_data['测评日期'], child_data['综合得分'],
             marker='o', linestyle='-', linewidth=2,
             label=f"儿童{i}")

plt.title('综合得分康复趋势（改善最明显的10名儿童）', fontsize=24)
plt.xlabel('测评日期', fontsize=20)
plt.ylabel('综合得分', fontsize=20)
plt.xticks(rotation=45, fontsize=15)  # 新增字号设置
plt.yticks(fontsize=15)  # 新增y轴刻度字号
...
plt.tick_params(axis='both', labelsize=15)  # 统一设置刻度字号

# 变化箱线图子图
plt.subplot(2, 1, 2)
sns.boxplot(y=results_df['变化值'])
plt.tick_params(axis='y', labelsize=15)  # 设置y轴刻度字号

# 3. 前后对比散点图
plt.scatter(results_df['首次得分'], results_df['末次得分'], alpha=0.7)
plt.xticks(fontsize=15)  # x轴刻度字号
plt.yticks(fontsize=15)  # y轴刻度字号
plt.plot([0, 100], [0, 100], 'r--')  # 对角线
plt.title('首次vs末次测评得分对比', fontsize=24)
plt.xlabel('首次测评得分', fontsize=20)
plt.ylabel('末次测评得分', fontsize=20)
